...
#define DEFAULT_AP_NAME    PSTR("AziLamp")              // Имя точки доступа по умолчанию 
...
bool     useSoftAP = true;                 // использовать режим точки доступа
bool     wifi_connected = false;            // true - подключение к wifi сети выполнена  
bool     ap_connected = true;              // true - работаем в режиме точки доступа;
...
uint16_t   CURRENT_LIMIT = 800;                // лимит по току в миллиамперах, автоматически управляет яркостью (пожалей свой блок питания!) 0 - выключить лимит
